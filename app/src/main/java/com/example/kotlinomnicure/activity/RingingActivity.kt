...
class RingingActivity :BaseActivity() {
    private val log: Logger =
        LoggerFactory.getLogger(RingingActivity::class.java)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val caller_image_view: CircularImageView = findViewById<CircularImageView>(R.id.caller_image_view)
        ...
        if (intent.hasExtra("call")) {
            if (intent.getStringExtra("call")
                ...
                callType = "Audio call"
                    ...
                    .equals(Constants.FCMMessageType.VIDEO_CALL,ignoreCase = true)
            ) {
                callType = "Video call"
            }
        }
        ...
        val callerTitle = """${if (intent.hasExtra("sos")) "SOS " + callType.lowercase(Locale.getDefault()) + " " else callType} from $providerName$pt"""
    ...
    protected fun initUIandEvent() {}

    protected fun deInitUIandEvent() {}
    ...
    private fun sendCallResponseMessage(messageType: String) {
        ...
        requestBody.setProviderId(PrefUtility().getProviderId(this)) //id key
    ...
    fun forwardToRoom() {
        ...
        intent.getStringExtra(ConstantApp().ACTION_KEY_CHANNEL_NAME)?.let {
            CurrentUserSettings().setmChannelName(it)
        }


        CurrentUserSettings().setmEncryptionKey("")
        val intent = Intent(this, CallActivity::class.java)
        intent.putExtra(ConstantApp().ACTION_KEY_CHANNEL_NAME,
    ...
    class ScreenReceiver : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            if (intent.action == Intent.ACTION_SCREEN_OFF) {
                ...
                if (intent.hasExtra("sos") && channel.equals(intent.getStringExtra(
                        ConstantApp().ACTION_KEY_CHANNEL_NAME), ignoreCase = true)
                ) {
