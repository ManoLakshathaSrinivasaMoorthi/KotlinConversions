...
class CensusHospitalListAdapter(hospitalRecyclerListener: CensusHospitalListViewAdapter.HospitalRecyclerListener, hospitalList: List<Hospital>?, selectedHosp: String?) : RecyclerView.Adapter<CensusHospitalListAdapter.ViewHolder>(),Filterable {
    ...
    private var hospitalListFiltered: List<Hospital>? = hospitalList
    ...
    fun setOnSearchResultListener(onSearchResultListener: CensusWardListAdapter.OnSearchResultListener) {
    ...
    private fun setHospitalList(holder: ViewHolder, position: Int) {
        ...
        holder.itemBinding.txtAddress.text = hospital.getSubRegionName()
            ...
            override fun performFiltering(charSequence: CharSequence): FilterResults {
                ...
                hospitalListFiltered = if (charString.isEmpty()) {
                    ...
                    for (hospital in hospitalList!!) {
                        if (hospital.getName()?.toLowerCase(Locale.ROOT)?.contains(charString.toLowerCase(Locale.ROOT)) == true ||
                            hospital.getSubRegionName()?.toLowerCase(Locale.ROOT)?.contains(charString.toLowerCase(Locale.ROOT)) == true
                        ) {
    ...
    class ViewHolder(itemBinding: ItemChildCensusListBinding) : RecyclerView.ViewHolder(itemBinding.root) {
        var itemBinding: ItemChildCensusListBinding = itemBinding
