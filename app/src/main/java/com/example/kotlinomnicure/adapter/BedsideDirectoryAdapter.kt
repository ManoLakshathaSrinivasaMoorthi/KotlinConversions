...
class BedsideDirectoryAdapter(
...
) : RecyclerView.Adapter<BedsideDirectoryAdapter.ViewHolder>() {
    var providerList: List<Provider>? = null
    var context: Context? = null
    var getActivity: Activity? = null
    var listen: DirectoryClickListener? = null

    fun BedsideDirectoryAdapter(getActivity: Activity, context: Context?, objects: List<Provider>) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (provider.getHospital() != null) {
            holder.itemBinding.role.setText(provider.getHospital())
        } else {
        ...
        } else if (provider.getStatus().equals(Constants.ProviderStatus.Active.toString(),ignoreCase = true)
            || provider.getStatus().equals(Constants.ProviderStatus.AutoLock.toString(),ignoreCase = true)
        ) {
